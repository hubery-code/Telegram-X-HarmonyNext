...
| CORE-005 授权状态机 reducer | 主会话 | 2026-09-09 | authorizationState → AccountState 纯转换 + 全分支 fixture 测试；区域：`core/account/`（AuthorizationState 占位处） |
| BRG-006 生产桥 adapter | 主会话 | 2026-09-09 | 用 libtdcore_napi.so 实现 TdNativeBridgeLike，替换 entry 临时 TdBridge；区域：`platform/tdcore-bridge/`、`entry/` 装配 |
...
| CORE-002 TdGateway+RequestRegistry | 2026-09-09 ✅ | `core/td_gateway`（`@tgx/td-gateway`）：RequestRegistry（@extra.requestId 注入、Promise 恒回 Result 不 reject、超时 10s 可配、取消、迟到丢弃计数、7 项指标）+ OrderedEventRouter（clientId+sequence 单调校验、乱序上报不静默、重复幂等）；FakeBridge 单测 53/53；CI 全绿；契约/交接点见 README；commit `4f1194e` |
